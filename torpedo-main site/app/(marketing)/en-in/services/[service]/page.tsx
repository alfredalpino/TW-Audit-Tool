...
import { citiesBySlug } from '@/lib/seo/cities';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const path = `/en-in/services/${service.slug}`;
  return {
    title: `${service.name} Agency India | Torpedo Web`,
...
export default async function EnInServiceDetailPage({ params }: Props) {
  const { service: serviceSlug } = await params;
  const service = servicesBySlug.get(serviceSlug);
  if (!service) notFound();
  ...
  const pageUrl = toAbsoluteUrl(`/en-in/services/${service.slug}`);

  const schemaGraph = [
    buildBreadcrumb([
      { name: 'Home', url: toAbsoluteUrl('/en-in') },
      { name: 'Services', url: toAbsoluteUrl('/en-in/services') },
    ...
    buildService({
      ...
      description: service.intro,
      url: pageUrl,
      areaServed: ['India', ...relatedCities.map((city) => city.name)],
    }),
  ...
  return (
    <main id="main-content" className="flex min-h-screen w-full flex-col pt-20">
      ...
      <div className="container mx-auto w-full max-w-6xl px-6 pb-24 pt-12 md:px-12">
        <p className="mb-4 text-xs font-semibold uppercase tracking-[0.16em] text-torpedo-orange">India Service Stack</p>
        ...
        <section className="mb-10 rounded-xl border border-gray-200 bg-white p-6">
          <h2 className="mb-4 text-xl font-semibold text-torpedo-dark">City Coverage</h2>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {relatedCities.map((city) => (
              <Link
                key={city.slug}
                href={`/en-in/${city.slug}`}
