    ...
    <section className="tw-panel min-w-0 p-5 md:p-6">
      ...
      <div className="mt-6 grid min-w-0 gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {entries.map(({ key, label, data }) => (
          <div
            key={key}
            className="min-w-0 rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--bg-muted)] p-4"
          >
            <p className="tw-contain-text font-mono text-[10px] uppercase tracking-wider text-[var(--fg-tertiary)]">
              {label}
            </p>
            <p className="tw-contain-text mt-2 font-display text-lg font-bold text-[var(--brand)] sm:text-xl">
              {formatImpactMetricDisplay(data?.label ?? "", data?.range)}
            </p>
            {data?.narrative && (
              <p className="tw-contain-text mt-2 text-sm leading-relaxed text-[var(--fg-secondary)]">
                {data.narrative}
              </p>
            )}
          </div>
