...
export function AuditReportDocument({ data }: { data: AuditReportData }) {
  ...
  const preparedFor = data.organizationName?.trim();

  return (
    <Document
      title={`Torpedo Audit — ${data.url}`}
      author="Torpedo Web"
      subject={preparedFor ? `Prepared for ${preparedFor}` : "Website intelligence audit"}
    >
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          <Text style={styles.brand}>Torpedo Website Intelligence</Text>
          <Text style={styles.poweredBy}>Powered by Torpedo Web</Text>
          <Text style={styles.title}>Audit Report</Text>
          {preparedFor ? (
            <Text style={styles.orgLine}>Prepared for {preparedFor}</Text>
          ) : null}
        ...
        <Text style={styles.footer} fixed>
          <Text style={styles.footerBrand}>Torpedo Web</Text>
          {" · torpedoweb.org · audit.torpedoweb.org · "}
          {data.completedAt
